...
olive = Sauce("Ek malzeme", "Zeytin", 5)
mushroom = Sauce("Ek malzeme", "Mantar", 5)
gcheese = Sauce("Ek malzeme", "Keçi Peyniri", 10)
beef = Sauce("Ek malzeme", "Et", 15)
onion = Sauce("Ek malzeme", "Soğan", 5)
corn = Sauce("Ek malzeme", "Mısır", 5)

total_order = 0
pizza_list = [classic, margherita, turkpizza, plainpizza]
sauce_list = [olive, mushroom, gcheese, beef, onion, corn]
...
def sauce_total(s_choice):
    s_choice = int(s_choice)
    return sauce_list[s_choice].get_cost()
    ...
    while True:
        ...
        if (p_choice == "Q") or (p_choice == "q"):
            ...
            if piece in ["1", "2", "3"]:
                ...
                i = 0
                for i in range(1, (len(sauce_list)+1)):
                    print(i,f"{sauce_list[i-1]}",sauce_list[i-1].get_cost(),"TL","\t",end="")
                    i += 1
                ...
                while (s_choice !="0"):
                    ...
                    if s_choice == "0":
                        ...
                        if sauce_list[int(s_choice)] not in s_list:
                            print(f"\nEk malzeme olarak {sauce_list[s_choice]} seçtiniz.")
                            s_list.append(sauce_list[s_choice])
                            total_order += (sauce_total(s_choice)*piece)
